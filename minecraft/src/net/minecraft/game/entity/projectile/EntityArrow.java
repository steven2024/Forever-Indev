...
public class EntityArrow extends Entity {
    ...
    public final void onEntityUpdate() {
        ...
        if (hit != null) {
            if (hit.entityHit != null) {
                ...
                if (this.detonationLevel > 0) {
                    EntityLiving player = this.worldObj.playerEntity; // Directly reference the player
                    
                    // Check if the explosion is directly below the player
                    boolean isBelowPlayer = false;
                    ...
                    // Create explosion with conditions
                    if (isBelowPlayer) {
                        this.worldObj.playSoundAtEntity(this, "random.explode", 1.0F, 1.0F);
                        this.worldObj.spawnParticle("hugeexplosion", (float) this.posX, (float) this.posY, (float) this.posZ, 1.0F, 0.0F, 0.0F);
                    } else {
        ...
        if (this.handleWaterMovement()) {
            for (int i = 0; i < 4; ++i) {
                this.worldObj.spawnParticle("bubble", (float) this.posX - this.motionX * 0.25F, (float) this.posY - this.motionY * 0.25F, (float) this.posZ - this.motionZ * 0.25F, this.motionX, this.motionY, this.motionZ);
            }
            drag = 0.8F;
        }
